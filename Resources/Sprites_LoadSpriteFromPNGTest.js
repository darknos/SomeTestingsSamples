...
// load texture atlas based on device
  	if (game.screen.width == 320 || game.screen.width == 480) { //iphone 2G,3G 3GS
		sprite = platino.createSpriteSheet({image:'graphics/RES_Sprites_LoadSpriteFromPNGTest/loadSprites_spritesSheet.xml'});
	}
  	else if(game.screen.width == 640 || game.screen.width == 960) { //iphone 4, 4S, 5, 5S (retina)
    	sprite = platino.createSpriteSheet({image:'graphics/RES_Sprites_LoadSpriteFromPNGTest/loadSprites_spritesSheet-hd.xml'});
	}
   	if(game.screen.width == 768 || game.screen.width == 1024) { //ipad 1, 2, mini
    	sprite = platino.createSpriteSheet({image:'graphics/RES_Sprites_LoadSpriteFromPNGTest/loadSprites_spritesSheet-ipad.xml'});
  	}
  	else if(game.screen.width == 1536 || game.screen.width == 2048) { //ipad 3, 4 (retina)
   		sprite = platino.createSpriteSheet({image:'graphics/RES_Sprites_LoadSpriteFromPNGTest/loadSprites_spritesSheet-ipadhd.xml'});
  	}
