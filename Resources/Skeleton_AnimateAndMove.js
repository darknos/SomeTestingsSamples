...
update = function() {
    ...
    if(posX > game.screen.height || posX < 0)
    {
...
function createSkeletonAtPosition(posX, posY)
{
    ...
    var suffix = "";
        ...
        mySkeleton = animo.createSkeletonWithFile("graphics/RES_Skeletons_LoadTest/skeletons/Officer_Officer" + suffix + ".json",
                                                        scene);
        game.screenSpriteT = platino.createSprite({color:'#ffffff', x: 0, y: 0, width:1500, height: 5100});
        scene.add(game.screenSpriteT);
        game.screenSprite = platino.createSprite({color:'#eeeeee', x: 0, y: 300, width:500, height: 500});
        scene.add(game.screenSprite);
        game.screenSpriteT.color(1, 0, 0);
        game.screenSprite.color(0, 0, 1);
       game.screenSpriteT.addChildNode(game.screenSprite);
       game.screenSprite.addChildNode(mySkeleton.batchNode);
       //game.screenSprite.scale(1.5, 1.5);
       //game.screenSpriteT.scale(0.5, 0.5);
       //game.screenSprite.anchorPoint = {x:0.5, y: 0.5};
       game.screenSprite.angle = -10;
       //mySkeleton.batchNode.rotate(180);
       ///game.screenSprite.rotate(-10);
        mySkeleton.setPosition(posX, posY);
