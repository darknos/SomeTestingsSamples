...
Skeleton.prototype.update = function(self)
{
    ...
    {//handle bone positions
        ...
        if(this.transitionTime)
        {
            ...
            Ti.API.info(beginFrame, positionFrames);
 ...
 * @return {Number} 
 */
Skeleton.prototype.getAngle = function()
{
    return this.batchNode.angle;
}

/**
 * @method setAngle
 * Rotate sceleton
 * @param {Number} val angle in degree.
 */
Skeleton.prototype.setAngle = function(val)
{
    this.batchNode.angle = val;
}
...
Skeleton.prototype.loadSprites = function(spritesInfo, spriteAtlastFileName)
{
	...
	this.batchNode = platino.createSpriteSheet({image:spriteAtlastFileName});
	...
	for(var i = 0; i < spritesInfo.length; i++)
	{
		...
		if(sprName)
		{
		    var newSprite = platino.createSpriteSheet({image:spriteAtlastFileName});
