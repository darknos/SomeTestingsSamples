...
(function () {
	...
	b2World.prototype.addEventListener = function(eventName, listener) {
		if (!eventName || !listener) return;
		var needsSetDestructionListener = false;
		var needsSetContactListener = false;
		var worldInstance = this;

		if (eventName === 'SayGoodbyeFixture' || eventName === 'SayGoodbyeJoint') {
			if (!this.activeDestructionListener) {
				this.activeDestructionListener = new b2DestructionListener;
				needsSetDestructionListener = true;
			}
			this.activeDestructionListener[eventName] = function(arg1) {
				var e = {};
				e.type = eventName;
				e.source = worldInstance;
				if (eventName === 'SayGoodbyeFixture') {
					e.fixture = arg1;
				} else if (eventName === 'SayGoodbyeJoint') {
					e.joint = arg1;
				};
				listener(e);
			}
		} else {
			if (!this.activeContactListener) {
				this.activeContactListener = new b2ContactListener;
				needsSetContactListener = true;
			}

			this.activeContactListener[eventName] = function(arg1, arg2) {
				var e = {};
				e.type = eventName;
				e.source = worldInstance;
				e.contact = arg1;
				if (eventName === 'PostSolve') {
					e.impulse = arg2;
				} else if (eventName === 'PreSolve') {
					e.oldManifold = arg2;
				};
				listener(e);
			};
		...
		if (needsSetDestructionListener) {
			this.SetDestructionListener(this.activeDestructionListener);
		}
		if (needsSetContactListener) {
			this.SetContactListener(this.activeContactListener);
		}
	}
	b2World.prototype.removeEventListener = function(eventName) {
		if (!eventName) return;
		if (eventName === 'SayGoodbyeFixture' || eventName === 'SayGoodbyeJoint') {
			if (!this.activeDestructionListener) return;
			if (this.activeDestructionListener[eventName]) {
				this.activeDestructionListener[eventName] = null;
			}
		} else {
			if (!this.activeContactListener) return;
			if (this.activeContactListener[eventName]) {
				this.activeContactListener[eventName] = null;
			}
		}  
	}
...
(function () {
	...
	b2Contact.prototype.Update = function (listener) {
		...
		if (wasTouching == false && touching == true) {
			if (listener.BeginContact) listener.BeginContact(this);
		}
		if (wasTouching == true && touching == false) {
			if (listener.EndContact) listener.EndContact(this);
		}
		if ((this.m_flags & b2Contact.e_sensorFlag) == 0) {
			if (listener.PreSolve) listener.PreSolve(this, this.m_oldManifold);
		}
	}
